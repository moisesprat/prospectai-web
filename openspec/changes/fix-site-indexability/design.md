...
- Commits the regenerated files back to `main` with a bot commit (e.g. `chore: regenerate sitemap and static content [skip ci]`, guarding against an infinite trigger loop) if and only if the generated content differs from what's committed.
...
- [Bot commits could loop or spam history] → Action only commits when generated content actually differs from what's on `main`; commit message includes `[skip ci]` (or equivalent) so it doesn't retrigger itself or unrelated CI.
